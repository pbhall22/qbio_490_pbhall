...
```{r 2.2}
## don't run this
#setwd("/Users/peytonhall/Documents/qbio_490_pbhall/week3_R")
...
```{r setup Exercise 2.3}
    ...
    knitr::opts_knit$set(root.dir = normalizePath("/Users/peytonhall/Documents/qbio_490_pbhall")) 
```
...
string_var <- "This is qbio 490 fall 2023 class"
char_var <- "c"
...
age <- 20
birthday <- "04/22/2003"
three_fav_colors <- c("green", "blue", "yellow")
...
average <- 0
while (average <= 10) {
  average <- mean(c(1, 10, x))
  x <- x + 1
}

cat("Final average:", average, "\n")
cat("Value of x when average is greater than 10:", x, "\n")
```
...
mtcars[7,4]
rownames(mtcars)
mtcars[c("Duster 360", "Lincoln Continental", "Maserati Bora"), c("mpg", "cyl", "hp")]
mpg_mask <- ifelse(mtcars$mpg > 10, T, F)
```
...
mtcars$transmission
transmission_mask <- ifelse(mtcars$transmission == "automatic", T, F)
transmission_mask
...
```
  ...
  To get extra credit, email completed assignments to "jam76378@usc.edu" when prompted. You will receive 0.25 points for completion of any swirl exercise, with a max of 5 points to be added to your homework grade.
